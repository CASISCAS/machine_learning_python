...
class LinReg(object):
    ...
    def predict(self, X):
        ...
        """
        return np.insert(X, 0, 1, axis=1).dot(self.theta)
...
def demo():
    ...
    data = np.genfromtxt('Data/bloodpressure.csv', delimiter=',', skip_header=1)
    X = data[:, 1:]
    y = data[:, 0]
    ...
    test = np.genfromtxt('Data/bloodpressure.csv', delimiter=',', skip_header=1)
    X_test = test[:, 1:]
    y_test = test[:, 0]
